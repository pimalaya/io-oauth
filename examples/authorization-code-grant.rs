...
fn main() {
    ...
    let request_params = AuthorizationRequestParams {
        ...
        state: Some(AuthorizationRequestParams::new_random_state().into()),
    };

    auth_uri.set_query(Some(&request_params.to_string()));
    ...
    if !request_params.matches_response_state(&response_params) {
    ...
    let params = AccessTokenRequestParams::new(response_params.code, redirect_uri, client_id);
