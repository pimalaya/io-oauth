//! Module dedicated to the [`AuthorizationCodeGrant`] coroutine.
...
impl<'a> RefreshAccessTokenParams<'a> {
    pub fn new(refresh_token: impl Into<SecretString>) -> Self {
        Self {
    ...
    pub fn with_scope(mut self, scope: impl Into<Cow<'a, str>>) -> Self {
        self.scopes.insert(scope.into());
        self
    }

    pub fn with_scopes(mut self, scopes: impl Iterator<Item = impl Into<Cow<'a, str>>>) -> Self {
        for scope in scopes {
            self.scopes.insert(scope.into());
        }
        self
