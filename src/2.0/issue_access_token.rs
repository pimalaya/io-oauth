...
pub enum Oauth20IssueAccessTokenErrorCode {
    ...
    /// SHOULD repeat the device access token request to the token
    /// endpoint, after waiting at least the polling interval.
    ///
    ...
    /// The device code has expired, and the device authorization
    /// session has concluded.  The client MAY commence a new device
    /// authorization request but SHOULD wait for user interaction
    ...
    ///
    /// Non-standard code sent by Microsoft Entra in place of the RFC
    /// 8628 "access_denied".
    AuthorizationDeclined,

    /// The device code sent to the token endpoint was not recognized.
    ///
